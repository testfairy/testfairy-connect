	...
	this.jira.listIssueTypes()
		.then(function (result) {
			...
			if (result) {
				...
				jiraIssueTracker.eventEmitter.emit('trackerError', "No issue types loaded");
			}
		})
		.catch(function (error) {
			jiraIssueTracker.eventEmitter.emit('trackerError', error);
		});
};
...
jiraIssueTracker.updateIssue = function (options, callback) {
	...
	this.jira.updateIssue(options.issueKey, issue)
		...
		.catch(function (error) {
			jiraIssueTracker.eventEmitter.emit('trackerError', ['Error updating issue:', options, error]);
		});
};
...
function mapJiraIssuesResponse(jiraResponse) {
	var results = [];
	for (var i in jiraResponse.issues) {
		if (jiraResponse.issues.hasOwnProperty(i)) {
			var issue = jiraResponse.issues[i];
			var result = {
				'issueKey': issue.key,
...
jiraIssueTracker.prepareCustomFields = function () {
	...
	if (jiraIssueTracker.options.customFields) {
		var projectsCount = jiraIssueTracker.options.customFields.length;
		for (var i = 0; i < projectsCount; i++) {
			var project = jiraIssueTracker.options.customFields[i];
			this.customFields[project.projectKey] = {};
			var fieldsCount = project.fields.length;
