	...
	if (!this.jira) {
		...
		this.listProjects(function (res) {
			if (res.projects.length > 0) {
				...
				jiraIssueTracker.eventEmitter.emit('trackerError', ["Projects list is empty. Please check your configuration or user permissions", options, sanitizeJiraError(res.error)]);
			}
		});
	}
};
...
jiraIssueTracker.createIssue = function (options, callback) {
	...
	this.jira.addNewIssue(issue)
		...
		.catch(function (error) {
			jiraIssueTracker.eventEmitter.emit('trackerError', ['Error creating issue', options,sanitizeJiraError(error)]);
		});
};

function sanitizeJiraError(error) {
	const copy = {...error};
	if (copy.options) {
		delete copy.options.auth;
	}
...
jiraIssueTracker.listProjects = function (callback) {
	...
	this.jira.listProjects()
		...
		.catch(function (error) {
			console.log("JIRA returned an error: " + JSON.stringify(sanitizeJiraError(error)));
...
jiraIssueTracker.createMeta = function (options, callback) {
	...
	this.jira.createMeta(options.projectKey)
		...
		.catch(function (error) {
			jiraIssueTracker.eventEmitter.emit('trackerError', ['Error fetching createmeta', options, sanitizeJiraError(error)]);
		});
};
