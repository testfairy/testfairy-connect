...
tfsIssueTracker.createIssue = function (options, callback) {
    ...
    exec(command, function (error, stdout) {
        if (error) {
            tfsIssueTracker.logger.error('Error creating issue:', options, error);
        } else {
...
tfsIssueTracker.updateIssue = function (options, callback) {
    ...
    exec(command, function (error, stdout, stderr) {
        //parse stdout
        if (error) {
            tfsIssueTracker.logger.error('Error updating issue:', options, error);
        } else if (stdout.toString().indexOf('updated') < 0) {
            tfsIssueTracker.logger.error('Error updating issue:', options, stdout, stderr);
        } else {
...
tfsIssueTracker.listProjects = function (callback) {
    ...
    var command = 'PowerShell -ExecutionPolicy ByPass -File ' + __dirname +  '\\listtfsprojects.ps1 -collectionUri ' + tfsIssueTracker.options.URL;
    this.logger.info(command);
    exec(command, function (error, stdout, stderr) {
        var outputLines,
            names = [],
        ...
        //parse stdout
        if (error) {
            tfsIssueTracker.logger.error('Error listing projects:', error);
        } else if (stdout.toString().trim().length === 0) {
            tfsIssueTracker.logger.error('No projects found');
        } else {
            outputLines = stdout.split('\n');
            for (i = 0; i < outputLines.length; i++) {
                if (outputLines[i].length) {
                    names.push(outputLines[i].trim());
                }
            }
...
tfsIssueTracker.listIssues = function (options, callback) {
    ...
    exec(command, function (error, stdout, stderr) {
        if (error) {
            tfsIssueTracker.logger.error('Error getting issue details:', options, error, stderr);
        } else {
