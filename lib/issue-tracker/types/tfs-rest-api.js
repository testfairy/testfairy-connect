...
var tfsRestApiIssueTracker = {
	...
	/**
	 * Return "open" or "closed" based on System.State value
	 *
	 * @param status
	 * @returns {*}
	 */
	convertWorkitemState: function(status) {
	...
	updateIssue: function(options, callback) {

		tfsIssueTracker.eventEmitter.emit('trackerError', ['Updating issue not implemented for tfs-rest-api:', options, error]);
	},

	listIssues: function(options, callback) {

		this.logger.log("Listing issues " + options.ids);

		var ids = [];
		for (var i=0; i<options.ids.length; i++) {
			// keys in testfairy are written as 'workitemtype id'
			// so we might get "User Story 16"
			var tokens = options.ids[i].split(" ");
		...
		var url = this.options.URL + "/_apis/wit/workitems?ids=" + ids.join(",");
		this.get(url, function(body, error) {
			...
			for (var i=0; i<value.length; i++) {
				var workitem = value[i];

				// status in TFS ("New", "Resolved"), in TestFairy
				var status = that.getField(workitem.fields, "System.State");
				var issueKey = that.getField(workitem.fields, "System.WorkItemType") + " " + workitem.id;

				var result = {
					issueKey: issueKey,
					status: that.convertWorkitemState(status),
					summary: that.getField(workitem.fields, "System.Title"),
					description: that.getField(workitem.fields, "System.Description")
				};
	...
	formatDescription: function(body)
	{
		...
		var re = /\\\\ (!)(https:[^|!]+)(|[^!]*)?(!)/g;
		body = body.replace(re, '<br /><img src="$2" style="max-width: 300px;" />');
